...
public class Test_organisation 
{
	public static void main (String[] args)
	{
		...
		case 4:
			...
			boolean empfound = false;//setting the flag
			try {
			for (int i= 0; i<selection.length;i++) // checking so that i do not traverse more than the size of array
			{
				if(selection[i] != null) //array value isn't a null value
				{
					if (selection[i].getid()==id)//Value of user mentioned id and the auto generated matches
					{
						empfound=true;
						selection[i].setbasic(selection[i].getbasicsalary()+inc);// basic +increment
						break;
			...
			if(!empfound){
				throw new CustomException("Id does not exists");//used only for 1 line exception in rest cases i.e where conditions are more you need to have separate util cases.
			}
			}
